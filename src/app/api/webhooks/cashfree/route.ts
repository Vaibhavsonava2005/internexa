...
export async function POST(req: Request) {
  try {
    ...
    if (!customer?.customer_email) {
      ...
      return NextResponse.json({ error: 'Missing customer_email' }, { status: 200 }); 
    }
    ...
    if (amount >= 98 && amount <= 100) {
      // Find the specific application the user initiated payment for
      const { data: applications, error: appError } = await supabaseAdmin
        .from('applications')
        .select('*, internships(title, duration)')
        .eq('clerk_id', user.clerk_id)
        .eq('status', 'Payment Processing') // Pre-flight intent flag
        .order('updated_at', { ascending: false })
        .limit(1);

      if (appError || !applications || applications.length === 0) {
        console.error("No Payment Processing application found for onboarding:", email);
      ...
      console.log(`Onboarding automated for ${email}, App ID: ${application.id}`);
    } 
    // Amount ~ 199 is Certification
    else if (amount >= 198 && amount <= 200) {
      // Find specific FastTrack application
      const { data: applications, error: appError } = await supabaseAdmin
        .from('applications')
        .select('*, internships(title, duration)')
        .eq('clerk_id', user.clerk_id)
        .eq('status', 'FastTrack Processing')
        .order('updated_at', { ascending: false })
        .limit(1);

      if (appError || !applications || applications.length === 0) {
        console.error("No FastTrack Processing application found for:", email);
      ...
      console.log(`Certification automated for ${email}, App ID: ${application.id}`);
    }
